# -*- coding: utf-8 -*-
"""
KINDS 이슈 → 대표 기사 상세(content) 수집 → (필요시) 원문 폴백 → UTF-8 파일 저장

업데이트 요약:
- ✅ 폴백 성공 + 제목 유사도 낮고 폴백 제목이 '경향신문'(등 사이트명/매체명)인 경우 → 저장 스킵
- ✅ ISSUE_PROVIDERS_FILTER / SAVE_ONLY_PROVIDERS에 구현된 모든 언론사 포함
- ✅ 경향 AMP 완화 클린업 및 셀렉터 보강 유지
"""
...
# 지금까지 구현한 모든 매체(저장/수집 제한)
ISSUE_PROVIDERS_FILTER = [
    "MBC", "KBS", "SBS", "국민일보",
    "조선일보", "중앙일보", "동아일보",
    "한겨레", "경향신문",
]
SAVE_ONLY_PROVIDERS = set(ISSUE_PROVIDERS_FILTER)
...
# 폴백 비활성화(필요시 추가)
DENY_FALLBACK_HOSTS = {
    # 예: "www.chosun.com", "www.joongang.co.kr", ...
}
...
def crawl_article(url: str, timeout: int = 15) -> Optional[Tuple[str, str]]:
    ...
    try:
        ...
        for i, u in enumerate(candidates):
            ...
            print(f"  ↳ GET {u} status={r.status_code} bytes={len(r.content) if r.content else 0}")
            ...
            try:
                ...
                if amp and amp.get("href"):
                    ...
                    print(f"  ↳ GET {amp_url} status={r2.status_code} bytes={len(r2.content) if r2.content else 0}")
        ...
        try:
            ...
            with open(os.path.join("_debug", "last_fail.html"), "wb") as f:
                f.write(r.content if 'r' in locals() and r.content else b"")
            print("  ↳ saved _debug/last_fail.html")
        except Exception:
            pass
        return None
...
# KINDS API
# ─────────────────────────────────────────────────────────────
def kinds_issue_ranking(date: str) -> List[Dict]:
    url = "https://tools.kinds.or.kr/issue_ranking"
    payload = {"access_key": ACCESS_KEY, "argument": {"date": date, "provider": ISSUE_PROVIDERS_FILTER or []}}
    r = requests.post(url, json=payload, headers=KINDS_HEADERS, timeout=15)
    r.raise_for_status()
    return r.json().get("return_object", {}).get("topics", [])
...
def kinds_fetch_news_detail(news_ids: List[str], fields: Optional[List[str]] = None) -> Dict:
    ...
    payload = {"access_key": ACCESS_KEY, "argument": argument}
...
def looks_truncated(text: str) -> bool:
    if not text:
        return True
    if len(text) < MIN_REASONABLE_LEN:
        return True
    tail = text.strip()[-40:]
...
# 수집 + 폴백 + 저장 스킵 로직
# ─────────────────────────────────────────────────────────────
def collect_articles_with_fallback(date: str, max_topics: int = 3, per_topic_docs: int = 1) -> List[Dict]:
    topics = kinds_issue_ranking(date)
    out: List[Dict] = []
    ...
    for t in topics[:max_topics]:
        ...
        if not cluster:
            out.append({"topic": topic_name, "status": "no_cluster"})
            continue

        detail = kinds_fetch_news_detail(
            news_ids=cluster,
            fields=["news_id","title","content","content_original","provider","published_at","provider_link_page","url","link",'byline','category']
        )
        docs = (detail.get("return_object", {}) or {}).get("documents", [])
        if not docs:
            out.append({"topic": topic_name, "status": "no_docs", "news_ids": cluster})
            continue
        ...
        for d in docs:
            ...
            if SAVE_ONLY_PROVIDERS and provider not in SAVE_ONLY_PROVIDERS:
                print(f"  ↳ SKIP: provider={provider} (필터: {','.join(SAVE_ONLY_PROVIDERS)})")
                continue
            ...
            if looks_truncated(final_body):
                if not url:
                    print(f"  ↳ SKIP FALLBACK: url 없음 (news_id={d.get('news_id')}, provider={provider})")
                else:
                    if urlparse(url).netloc.lower() in DENY_FALLBACK_HOSTS:
                        print(f"  ↳ FALLBACK DISABLED (policy) for {url}")
                    else:
                        crawled = crawl_article(url)
                        if crawled:
                            crawled_body, crawled_title = crawled
                            crawled_len = len(crawled_body)
                            base_len = len(final_body)
                            cond1 = crawled_len > base_len
                            cond2 = (base_len <= 250 and crawled_len >= 400)
                            if cond1 or cond2:
                                used_fallback = True
                                ...
                                if sim < 0.35 and is_generic_title(crawled_title or "", provider):
                                    # ★ 요구사항: 이런 경우 저장 스킵
                                    print(f"[{topic_name}] '{title[:30]}...' → ❌ 저장 스킵: "
                                          f"제목 미스매치(sim={sim:.2f}) & 폴백 제목이 매체명/사이트명('{(crawled_title or '').strip()}')  ({provider})")
                                    out.append({
                                        "topic": topic_name,
                                ...
                                if crawled_title and not is_generic_title(crawled_title, provider) and sim < 0.35:
                                    print(f"  ↳ TITLE SWAP (sim={sim:.2f}) '{title[:20]}...' → '{crawled_title[:20]}...'")
                                    final_title = crawled_title
                            else:
                                print(f"  ↳ FALLBACK DISCARD: crawled_len={crawled_len} <= base_len={base_len}")
                        else:
                            print("  ↳ FALLBACK FAILED: crawled=None")

            # 3) 폴백 실패 + 본문 짧음 → 저장 스킵
            if not used_fallback and len(final_body) <= MIN_KINDS_LEN_TO_SAVE_IF_NO_FALLBACK:
                print(f"[{topic_name}] '{title[:30]}...'  KINDS_len={len(body_kinds):5d}  "
                      f"→ ❌ 저장 스킵: 폴백 실패 & 본문 짧음(≤{MIN_KINDS_LEN_TO_SAVE_IF_NO_FALLBACK})  ({provider})")
                out.append({
                    "topic": topic_name,
                    "news_id": d.get("news_id",""),
                    "title": title,
                    ...
                    "published_at": published_at,
                    "url": url,
                    "status": "skipped_short_kinds"
                })
                continue

            print(f"[{topic_name}] '{final_title[:30]}...'  "
                  f"KINDS_len={len(body_kinds):5d}  final_len={len(final_body):5d}  "
                  f"{'FALLBACK' if used_fallback else 'KINDS'}  ({provider})")
...
def save_articles(articles: List[Dict], save_dir: str = "articles") -> None:
    ...
    for a in articles:
        ...
        date_str = (a.get("published_at") or "")[:10] or "unknown-date"
        nid = (a.get("news_id") or "unknown")[:40]
        title_snip = safe_filename(a.get("title") or "article", 60)
        fname = f"{date_str}_{nid}_{title_snip}.txt"
        path = os.path.join(save_dir, fname)

        with open(path, "w", encoding="utf-8") as f:
            f.write(a.get("title","") + "\n")
            f.write(f"[{a.get('provider','')}] {a.get('published_at','')}\n")
            if a.get("url"):
                f.write(a["url"] + "\n")
            if a.get("byline"):
    ...
    print(f"\n저장 요약: 저장 {saved}건, 스킵 {skipped}건 → 폴더: {os.path.abspath(save_dir)}")
...
ACCESS_KEY = os.getenv("KINDS_ACCESS_KEY", "your_key")
...
if __name__ == "__main__":
    ...
    save_articles(articles, save_dir=SAVE_DIR)
